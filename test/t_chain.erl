...
-type optional_params() ::
    #{
        vars               => map(),
        init_balance_hnt   => non_neg_integer(),
        init_balance_dc    => non_neg_integer(),
        init_balance_hst   => non_neg_integer(),
        num_init_users     => non_neg_integer(),

        user_master        => t_user:t(),
        user_app           => t_user:t(),
        users_with_hnt     => [t_user:t()],
        users_with_hst     => [t_user:t()],
        users_with_dc      => [t_user:t()],
...
start(Cfg, Opts) when is_list(Cfg), is_map(Opts) ->
    ...
    InitBalanceHNT = maps:get(init_balance_hnt, Opts, 5000),
    InitBalanceDC  = maps:get(init_balance_dc , Opts, 5000),
    InitBalanceHST = maps:get(init_balance_hst, Opts, 5000),
    ...
    UsersGetOrGenerate =
        fun(K, N) -> maps_get_or(K , Opts, fun() -> t_user:n_new(N) end) end,

    UsersWithHNT     = UsersGetOrGenerate(users_with_hnt    , NumInitUsers),
    UsersWithHST     = UsersGetOrGenerate(users_with_hst    , NumInitUsers),
    UsersWithDC      = UsersGetOrGenerate(users_with_dc     , NumInitUsers),
    UsersAsGateways  = UsersGetOrGenerate(users_as_gateways , NumInitUsers),
    UsersInConsensus = UsersGetOrGenerate(users_in_consensus, NumInConsensus),

    Addrs = fun (Us) -> [t_user:addr(U) || U <- Us] end,

    GenesisTxns =
        [genesis_txn_vars(Vars, t_user:key_pair(MasterUser))] ++
        genesis_txns_pay_hnt([{A, InitBalanceHNT} || A <- Addrs(UsersWithHNT)]) ++
        genesis_txns_pay_dc( [{A, InitBalanceDC}  || A <- Addrs(UsersWithDC)]) ++
        genesis_txns_pay_sec([{A, InitBalanceHST} || A <- Addrs(UsersWithHST)]) ++
...
commit(Chain, ConsensusMembers, [_|_]=Txns0) ->
    ...
    case blockchain_txn:validate(Txns, Chain) of
        {_, []} ->
            Block = make_block(Chain, ConsensusMembers, Txns),
...
make_block(Chain, ConsensusMembers, Txns) ->
    {ok, HeadBlock} = blockchain:head_block(Chain),
...
genesis_txns_consensus(AddressesOfGateways, ElectionVersion) ->
    ...
    case ElectionVersion of
        gt_5 ->
            [blockchain_txn_gen_validator_v1:new(Addr, Addr, ?bones(10000)) || Addr <- AddressesOfGateways];
        lt_5_or_unspecified ->
            [blockchain_txn_gen_gateway_v1:new(Addr, Addr, Loc, 0) || {Addr, Loc} <- lists:zip(AddressesOfGateways, Locations)]
    end.
